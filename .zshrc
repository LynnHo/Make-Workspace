...
rrm()( if [ -f "$1" ] || [ -h "$1" ]; then rm "$1"; elif [ -d "$1" ]; then local temp_dir=$(mktemp -d); rsync -av --delete "$temp_dir/" "$1"; rmdir "$temp_dir"; rmdir "$1"; else echo "$1 is not a valid file, directory, or symlink"; return  1; fi )
